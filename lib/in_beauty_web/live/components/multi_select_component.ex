defmodule InBeautyWeb.MultiSelectComponent do
  ...
  # TODO to finish multiselect
  def desktop(assigns) do
    ~H"""
      <div class="hidden md:block relative p-1 w-max">
        <p class="text-2xl my-auto break-all pb-2 "><%= @placeholder %>:</p>        
        <!-- DROPDOWN -->
        <ul id={"multiselect_dropdown#{@field}"}
          class="w-80 h-48 dark:bg-midnight-500 border-2 border-rose-300 rounded-2xl shadow-md overflow-y-auto"
          >
          <%= for variant <- @variants do %>
            <li class="group hover:bg-rose-100">
    ...
    <!--  mobile  -->    
      <div
        class="md:hidden relative p-1 w-full"
      >
        <p class="inline-block w-32 text-xl font-medium text-rose-100 my-auto mr-12 break-all"><%= @placeholder %>:</p>
        <div class="inline-flex items-center justify-center z-10 w-40 h-16 bg-white dark:bg-midnight-500 border-2 border-rose-300 rounded-2xl relative">
  ...
  def desktop_with_dropwown(assigns) do
    ~H"""
    <section class="relative max-w-xs xs:max-w-full xs:w-auto">
      <div class="hidden md:grid grid-cols-2 relative p-1 w-max">
        <p class="w-64 text-3xl my-auto break-all"><%= @placeholder %>:</p>
        <div class="w-80 h-12 bg-white dark:bg-midnight-500 border-2 border-rose-300 rounded-2xl relative h-auto">
          <div class="inline-block rounded-2xl overflow-x-auto">
          ...
          <div         
            ...
            class="inline-block p-2.5 h-10 w-10 ml-4"
            >
